...
var commandDocumentationByKey = map[string]CommandDocumentation{
	...
	"runs watch": {Samples: []CommandSample{
		sampleWithNote("axilio runs watch run_123", "<time>  span  sdk_call    Screen.observe  250ms\n<time>  INFO  output_log  <workflow output>\n✓ Run run_123 completed", "→ Run run_123 running — streaming session <session-id>", "Streams the session's telemetry — logs and completed spans — until the session ends. Exit 0 on completed, 1 on failed (with the run's error message), 7 on cancelled or interrupt. Watching a finished run replays its telemetry."),
