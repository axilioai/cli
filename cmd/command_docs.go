...
var commandDocumentationByKey = map[string]CommandDocumentation{
	...
	"sessions trace": {Samples: []CommandSample{
		...
		sampleWithNote("axilio sessions trace sess_123 --follow", "<archived trace table>\n<live rows as they arrive, COST \"-\">", "→ attached to live telemetry — Ctrl-C to stop", "Streams until the session-end frame; frames render exactly once across the archive/live seam. On an inactive session the archive prints and the command exits cleanly. With -o json the follow contract is newline-delimited JSON ending in a trace_end summary that carries the merged cost maps."),
	}},
